...
export function TranslationOutput() {
	...
	const { isError, isLoading, error, translatedText, wordMeanings } =
		useTranslator()
	...
	return (
		<div className="flex-1 flex flex-col">
			<div
				className={`${wordMeanings.length > 1 ? 'rounded-b-none border-b-0' : ''} 
				flex flex-col px-3 py-2 min-h-0 rounded-md border-solid mx-4 border-2 h-3/5`}
			>
			...
			{wordMeanings.length > 1 && (
				<div className="flex-col gap-2 px-3 py-2 min-h-0 rounded-b-md border-2 border-solid mx-4">
					<Label className="font-semibold mb-4">More translations:</Label>
					<div className="flex flex-col gap-2">
						{wordMeanings.map((meaning, index) => {
							if (meaning.translated.trim() === '') return null
							return (
								<div key={index} className="flex flex-row justify-between">
									<p>{meaning.translated}</p>
