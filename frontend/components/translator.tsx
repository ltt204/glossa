    ...
    async (text: string, target: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setTranslatedText(errData?.error || "Translation failed");
          return;
        }
        ...
        setTranslatedText("Could not connect to server");
      } finally {
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon-xs"
                  onClick={() => handleSpeak(sourceText, sourceLang === "auto" ? (detectedLang || "en") : sourceLang)}
                  disabled={!sourceText}
